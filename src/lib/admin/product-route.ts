...
const validateReferencedIds = async (
  ...
  if (missingIds.length) {
    if (options.allowMissing) {
      console.warn(`${label} validation pruned missing ids:`, missingIds)
      return {
        ids: dedupedIds.filter((id) => validIds.has(id)),
...
export async function createProduct(request: NextRequest) {
  ...
  if (
    parsed.data.deal_expires_at &&
    !(
      parsed.data.discount_price !== undefined &&
      parsed.data.discount_price !== null &&
      parsed.data.discount_price > 0 &&
      parsed.data.discount_price < parsed.data.price
    )
  ) {
    return jsonError('Set a valid discount price before adding a deal expiry.', 400)
  }
  ...
  if (!validatedCategoryIds.ids.length && !pendingCategoryRequestIds.length) {
  ...
  let sku = providedSku
    ...
    : await generateAutoSku(db, validatedCategoryIds.ids)
  ...
  const insertProduct = async (nextSlug: string, nextSku: string) =>
    db
      .from(PRODUCT_TABLE)
      .insert({
        ...
        deal_expires_at: parsed.data.deal_expires_at || null,
        sku: nextSku,
        sku_auto_generated: skuAutoGenerated,
        stock_quantity: parsed.data.stock_quantity ?? 0,
        initial_stock_quantity: Math.max(0, Number(parsed.data.stock_quantity ?? 0) || 0),
        status: createStatus,
        product_type:
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const insertResult = await insertProduct(slug, sku)
    ...
    const isSlugConflict =
      constraint.includes('products_slug_key') ||
      message.includes('products_slug_key') ||
      message.includes('duplicate key value') && message.includes('slug')
    if (isSlugConflict) {
      slug = await resolveUniqueProductSlug(db, `${baseSlug}-${Date.now().toString().slice(-3)}`)
      continue
    }

    const isSkuConflict =
      constraint.includes('products_sku_key') ||
      message.includes('products_sku_key') ||
      message.includes('duplicate key value') && message.includes('sku')
    if (isSkuConflict && skuAutoGenerated) {
      sku = await generateAutoSku(db, parsed.data.category_ids || [])
      continue
    }
    break
  }
...
export async function updateProduct(request: NextRequest, id: string) {
  ...
  const nextDealExpiresAt =
    updates.deal_expires_at !== undefined
      ? updates.deal_expires_at
      : existingProduct?.deal_expires_at
  const hasValidDealDiscount =
    nextDiscountPrice !== undefined &&
    nextDiscountPrice !== null &&
    Number(nextDiscountPrice) > 0 &&
    nextBasePrice > 0 &&
    Number(nextDiscountPrice) < nextBasePrice

  if (nextDealExpiresAt && !hasValidDealDiscount) {
    return jsonError('Set a valid discount price before adding a deal expiry.', 400)
  }
  ...
  if (validatedUpdateCategoryIds !== null && validatedUpdateCategoryIds.length === 0) {
  ...
  if (updates.status === 'publish') {
    ...
    if (validatedUpdateCategoryIds !== null) {
      nextCategoryCount = validatedUpdateCategoryIds.length
    } else {
  ...
  if (updates.deal_expires_at !== undefined) {
    updatePayload.deal_expires_at = updates.deal_expires_at ?? null
  } else if (updates.discount_price !== undefined && !hasValidDealDiscount) {
    updatePayload.deal_expires_at = null
  }
  ...
  if (updates.product_video_url !== undefined) {
    updatePayload.product_video_url = updates.product_video_url ?? null
  }
