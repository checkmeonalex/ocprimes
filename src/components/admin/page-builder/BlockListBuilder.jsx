...
function BlockItem({
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className={`group rounded-2xl border bg-white overflow-hidden transition ${
        isExpanded ? 'border-slate-900 shadow-[0_0_0_1px_theme(colors.slate.900)]' : 'border-slate-200 hover:border-slate-300'
      }`}
    >
      <div className="flex items-center gap-2.5 px-3 py-2.5">
        ...
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-500 [&>svg]:h-[18px] [&>svg]:w-[18px]">
          {typeMeta?.icon}
        </span>
        <button type="button" onClick={onToggle} className="flex-1 min-w-0 text-left">
          <p className="truncate text-[12.5px] font-bold text-slate-900">{typeMeta?.label || block.type}</p>
          {subtitle && <p className="truncate text-[10.5px] text-slate-400">{subtitle}</p>}
        </button>
        <button type="button" onClick={onDelete}
          className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-300 opacity-0 group-hover:opacity-100 hover:bg-rose-50 hover:text-rose-500 transition shrink-0 max-lg:opacity-100">
      ...
      {isExpanded && (
        <div className="border-t border-slate-100 bg-slate-50/60 px-4 pb-6 pt-4">
...
export default function BlockListBuilder({
  ...
  renderTopAddTrigger,     // optional (openPicker) => ReactNode — rendered above the list (only when blocks.length > 0; emptyState already gets its own affordance), for hosts that want the add action pinned near the top instead of/alongside the bottom trigger
  pickerFooter,
  className = '',
}) {
  ...
  return (
    <div className={className}>
      {blocks.length > 0 && renderTopAddTrigger && renderTopAddTrigger(openPicker)}
