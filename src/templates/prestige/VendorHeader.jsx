      ...
      >
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="relative flex items-center" style={{ height: HEADER_H }}>
            ...
            {/* LEFT — Menu/Store + Search */}
            <div className="flex flex-1 items-center gap-1 min-[390px]:gap-3 -ml-1">
              {showCollectionsMenu ? (
                <button type="button" onClick={() => setIsCollectionsOpen(true)}
                  className="flex items-center justify-center h-10 w-8 text-white/70 hover:text-white transition-colors"
                  aria-label="Menu">
            ...
            <div className="flex flex-1 items-center justify-end gap-1">
              ...
              {!isSearchOpen && (
                <div className="flex items-center gap-1 min-[390px]:gap-3">
                  <Link href="/account"
                    className="flex h-9 w-5 sm:w-9 items-center justify-center text-white/70 hover:text-white transition-colors" aria-label="Account">
                  ...
                  <Link href="/wishlist"
                    className="flex h-9 w-5 sm:w-9 items-center justify-center text-white/70 hover:text-white transition-colors" aria-label="Wishlist">
                  ...
                  <Link href="/cart"
                    className="relative flex h-9 w-5 sm:w-9 items-center justify-center text-white/70 hover:text-white transition-colors" aria-label="Cart">
              ...
              {!isSearchOpen && !canEditStorefront && canFollow && (
                <div className="relative hidden sm:block">
                  <button type="button" ref={followBtnRef} onClick={() => setIsConnectOpen((o) => !o)} disabled={isFollowLoading}
                    className={`flex rounded-full border px-3.5 py-1.5 text-xs font-semibold transition disabled:opacity-60 ${isFollowing ? 'border-white/20 bg-white/10 text-white/70 hover:bg-white/15' : 'border-white/40 bg-transparent text-white hover:border-white/70 hover:bg-white/10'}`}>
